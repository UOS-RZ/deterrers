...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-s', '--sync', action='store_true', help='Indicates whether to actually update the FW configuration')

    def __add_ips_to_addr_grps(self, fw : PaloAltoInterface, ips : list, grps : set):
        logger.warning("IPs %s are missing in AddressGroups %s", str(ips), str(grps))
        if self.sync:
            fw.add_addr_objs_to_addr_grps(ips, grps)

    def __rmv_ips_from_addr_grps(self, fw : PaloAltoInterface, ips : list, grps : set):
        logger.warning("IPs %s are wrongfully present in AddressGroups %s", str(ips), str(grps))
        if self.sync:
        ...
        match host.service_profile:
            case HostServiceContract.HTTP:
                if ipv4 not in fw_web_ipv4s:
                    self.__add_ips_to_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.HTTP,})
                if ipv4 in fw_ssh_ipv4s:
                    self.__rmv_ips_from_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.SSH,})
                if ipv4 in fw_open_ipv4s:
                    self.__rmv_ips_from_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.OPEN,})
                for ipv6 in ipv6s:
                    if ipv6 not in fw_web_ipv6s:
                        self.__add_ips_to_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.HTTP,})
                    if ipv6 in fw_ssh_ipv6s:
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.SSH,})
                    if ipv6 in fw_open_ipv6s:
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.OPEN,})

            case HostServiceContract.SSH:
                if ipv4 in fw_web_ipv4s:
                    self.__rmv_ips_from_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.HTTP,})
                if ipv4 not in fw_ssh_ipv4s:
                    self.__add_ips_to_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.SSH,})
                if ipv4 in fw_open_ipv4s:
                    self.__rmv_ips_from_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.OPEN,})
                for ipv6 in ipv6s:
                    if ipv6 in fw_web_ipv6s:
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.HTTP,})
                    if ipv6 not in fw_ssh_ipv6s:
                        self.__add_ips_to_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.SSH,})
                    if ipv6 in fw_open_ipv6s:
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.OPEN,})
            ...
            case HostServiceContract.MULTIPURPOSE:
                ...
                if ipv4 not in fw_open_ipv4s:
                    self.__add_ips_to_addr_grps(fw, [ipv4,], {PaloAltoAddressGroup.OPEN,})
                for ipv6 in ipv6s:
                    if ipv6 in fw_web_ipv6s:
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.HTTP,})
                    if ipv6 in fw_ssh_ipv6s:
                    ...
                    if ipv6 not in fw_open_ipv6s:
                        self.__add_ips_to_addr_grps(fw, [ipv6,], {PaloAltoAddressGroup.OPEN,})
            case HostServiceContract.EMPTY:
                if ipv4 in fw_web_ipv4s.union(fw_ssh_ipv4s).union(fw_open_ipv4s):
                    self.__rmv_ips_from_addr_grps(fw, [ipv4,], {addr_grp for addr_grp in PaloAltoAddressGroup})
                
                for ipv6 in ipv6s:
                    if ipv6 in fw_web_ipv6s.union(fw_ssh_ipv6s).union(fw_open_ipv6s):
                        self.__rmv_ips_from_addr_grps(fw, [ipv6,], {addr_grp for addr_grp in PaloAltoAddressGroup})
            case _:
                logger.warning("Invlaid service profile %s for %s", str(host.service_profile), str(ipv4))
                # TODO: maybe make also sure that ips are not present in any AddressGroup
                return
    ...
    def handle(self, *args, **options):
        # quick sanity check if service profiles and address groups are still up-to-date
        if not {sp for sp in HostServiceContract} == {HostServiceContract.EMPTY, HostServiceContract.HTTP, HostServiceContract.SSH, HostServiceContract.HTTP_SSH, HostServiceContract.MULTIPURPOSE}:
            logger.error("Service Profiles not up-to-date!")
            exit()
        if not {addrgrp for addrgrp in PaloAltoAddressGroup} == {PaloAltoAddressGroup.HTTP, PaloAltoAddressGroup.SSH, PaloAltoAddressGroup.OPEN}:
                    ...
                    with PaloAltoInterface(fw_username, fw_password, fw_url) as fw:
                        ...
                        for ipv4, host in ipam_hosts_total.items():
                            ...
                            match host.status:
                                ...
                                case HostStatusContract.BLOCKED | HostStatusContract.UNREGISTERED:
                                    self.__sync_host_blocked(host, ipam, fw, fw_web_ipv4s, fw_ssh_ipv4s, fw_open_ipv4s,fw_web_ipv6s,fw_ssh_ipv6s,fw_open_ipv6s)
                                case _:
                                    logger.warning("Invalid host status: %s", str(host.status))
