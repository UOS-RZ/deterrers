...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-s', '--sync', action='store_true', help='Indicates whether to actually update the scanner configuration')
                ...
                while True:
                    ...
                    with GmpVScannerInterface(v_scanner_username, v_scanner_password, v_scanner_url) as scanner:
                        ...
                        try:
                            ...
                            # if stash target exists use it, else use default target
                            target_uuid = scanner._GmpVScannerInterface__get_target_id(scanner.PERIODIC_TASK_STASH_TARGET_NAME)
                            if not target_uuid:
                        ...
                        for ipv4, host in ipam_hosts_total.items():
                            ...
                            match host.status:
                                case HostStatusContract.ONLINE:
                                    self.__sync_host_online(host, ipam, scanner, v_scanner_hosts)
                                ...
                                case HostStatusContract.BLOCKED | HostStatusContract.UNREGISTERED:
                                    self.__sync_host_blocked(host, ipam, scanner, v_scanner_hosts)
                                case _:
